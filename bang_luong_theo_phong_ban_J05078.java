...
class NhanVien {
    ...
    public String room(){
        return map.get(this.id.substring(3));
    }
...
public class bang_luong_theo_phong_ban_J05078 {
    public static void main(String[] args) {
        Scanner scn = new Scanner(System.in);
        int t = scn.nextInt();
        scn.nextLine();
        while(t-- >0){
            NhanVien.map.put(scn.next(), scn.nextLine().trim());
        }
        ...
        String nameRoom = NhanVien.map.get(scn.nextLine());
        System.out.println("Bang luong phong " + nameRoom + ":");
        for(NhanVien x : arr){
            if(nameRoom.equals(x.room())){
